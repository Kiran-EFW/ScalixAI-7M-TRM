#!/usr/bin/env python3
"""
Generate reasoning training data using Gemini API
"""
...
def main():
    ...
    parser.add_argument("--output-dir", type=str, default="data/reasoning",
    ...
    for batch in range(batches):
        ...
        examples = generator.generate_reasoning_examples(args.batch_size)
        all_examples.extend(examples)

        # Save intermediate results
        if (batch + 1) % 10 == 0:
            intermediate_file = output_dir / f"reasoning_data_intermediate_{batch + 1}.json"
            save_training_data(all_examples, str(intermediate_file))

    # Save final dataset
    final_file = output_dir / "reasoning_training_data.json"
    ...
    metadata = {
        ...
        "generation_config": {
            ...
            "temperature": 0.7,
            "topics": ["logic_puzzles", "mathematical_reasoning", "lateral_thinking"]
        }
    }
    ...
    logger.info(f"Generated {len(all_examples)} reasoning examples")
